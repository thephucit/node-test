 ...
 *
 */
const run = async (numConcurrentTasks, listTasks) => {
    const taskPromises = listTasks.splice(0, numConcurrentTasks);
    await Promise.all(taskPromises.map((p) => p())).then(async (res) => {
        // TODO:
